...
- `blueprints/automation/chris/pico_room_controller.yaml`
  ...
  - MIDDLE — **default**: short press recalls a **Scene**, long press saves a **temporary** favorite snapshot via `scene.create` + `snapshot_entities`; that temporary favorite is used until Home Assistant restarts/reloads scenes. Optional **Custom** mode uses your own short/long actions instead
